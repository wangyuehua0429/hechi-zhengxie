...
把旧库（宝塔每日全库备份 `gxhczx_db_2026-09-05_02-30-02_mysql_data.sql.gz`，或更早的 Navicat 导出）的主站内容迁进新库，并把历史图片抓回本地。
...
- 补抓的稿件走完全一样的后续流程（`fetch_media.py --verify` → `render` → `legacy_import.php --commit`），图片同样进媒体清单；
...
```bash
node tests/migrate-check.mjs        # 44 项：用 fixture 跑完整两段式（不联网），含多行 INSERT／gzip、删除同步、转归档
