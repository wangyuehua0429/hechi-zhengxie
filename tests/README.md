...
覆盖的 39 项检查包括：探活、43 个栏目的数量与栏目号集合、**每个栏目的列表项顺序与快照逐项一致**、单栏目元信息与列表、列表项字段（`date`／`datetime`／`views`／`hasBody`）、`/articles` 分页结构与 size 上限、详情页字段与正文图片、附件（两种写法结果一致）、检索命中与缺参数报错、首页 21 个模块、未知栏目／未知稿件／未知路径的 404 错误体，以及发布器的数据快照、精简索引、详情静态页与 sitemap。
...
覆盖 44 项：未登录访问 `/admin` 与 `/admin/articles` 是否被挡到登录页、登录页 CSRF 令牌、密码错误与缺令牌的提示、登录成功后的会话 Cookie、概览数据、稿件列表筛选、稿件编辑页带出原值、保存后数据真的进库（用 `/api/v1/article/{id}` 回读校验）、**草稿不对外可见、后台仍可检索**、令牌错误返回 400、恢复原稿、**新建稿件并立刻可被公开接口读到**、**上传附件（含文件可访问与删除后清理）**、**上传正文插图（正文出现 img 且登记为图集）**、**删除稿件（连文件一起清）**、栏目列表 43 行、栏目编辑页、一键发布后产物落盘（首页／栏目数据／详情静态页／sitemap）且详情页含标题、退出后再次被拦。
