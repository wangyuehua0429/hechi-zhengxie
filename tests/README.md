...
本目录七个脚本都是提交前检查，互不依赖；每行末尾的项数是 2026-09-12 实跑的结果：

| 脚本 | 检查对象 | 项数 | 前置 |
| --- | --- | --- | --- |
| `check-pages.mjs` | 前端页面（首页／栏目页／详情页），支持静态与接口两种数据源 | 19／20 | 静态模式无需依赖；接口模式需 PHP |
| `api-check.mjs` | 后端接口与静态化发布器 | 47 | 本机有 PHP（`brew install php`） |
| `admin-check.mjs` | 后台管理界面（登录／新建／编辑／上传／删除／发布／批量流转／栏目排序／首页四大类） | 209 | 本机有 PHP |
| `order-e2e.mjs` | 四类排序的端到端串联（后台 ↑↓ → 前台接口顺序） | 19 | 本机有 PHP |
...
- **E 按栏目置顶**：稿件编辑页勾选“在本栏目置顶”→ 栏目列表与首页模块同时排最前；取消置顶后**按发布时间落回原位**，不会把旧稿顶到最前。
