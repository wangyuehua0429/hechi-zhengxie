...
| `admin-check.mjs` | 后台管理界面（登录／新建／编辑／上传／删除／发布） | 本机有 PHP |
...
覆盖的 40 项检查包括：探活、43 个栏目的数量与栏目号集合、**栏目顺序与快照一致（即前台主导航顺序）**、**每个栏目的列表项顺序与快照逐项一致**、单栏目元信息与列表、列表项字段（`date`／`datetime`／`views`／`hasBody`）、`/articles` 分页结构与 size 上限、详情页字段与正文图片、附件（两种写法结果一致）、检索命中与缺参数报错、首页 21 个模块、未知栏目／未知稿件／未知路径的 404 错误体，以及发布器的数据快照、精简索引、详情静态页与 sitemap。
