...
本目录九个脚本都是提交前检查，互不依赖；每行末尾的项数是 2026-09-12 实跑的结果：
...
| `admin-check.mjs` | 后台管理界面（登录／新建／编辑／上传／删除／发布／批量流转／栏目排序与增删／首页四大类／徽标） | 240 | 本机有 PHP |
