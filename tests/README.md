...
| `admin-check.mjs` | 后台管理界面（登录／新建／编辑／上传／删除／发布／批量流转／栏目排序／首页四大类／徽标） | 223 | 本机有 PHP |
