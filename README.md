...
- **后台按首页四大类组织（2026-09-11 新增）**：侧栏改为“导航栏目 / 头条轮换 / 其他栏目 / 站内横幅”，分别对应首页的顶部导航条（18 个入口）、首屏轮播、正文 13 个内容模块与 7 处横幅位。首页内容模块由**静态快照改为按绑定栏目实时组装**（置顶在前 → 栏目内排序值 → 发布时间，不足时用快照兜底），编辑发稿即上首页；稿件编辑页的“置顶”改为**按栏目置顶**，首页模块与栏目页列表共用一套顺序；新增权限码 `home.manage` 与迁移 `003_home_sections.sql`。改动清单、数据模型与验证见 [docs/后台首页四大类管理说明.md](docs/后台首页四大类管理说明.md)。
