...
- **后端最小骨架（2026-09-11 新增）**：`backend/` 已从空骨架变为可运行的 PHP 轻量 CMS——MySQL／SQLite 双方言建表脚本（19 张业务表，另有迁移记录表 `schema_migrations`）、内容 REST 接口（`health`／`home`／`channels`／`channel-index`／`articles`／`article`／`attachments`／`search`，只出已发布且公开的内容）、静态化发布器（数据快照 + 全文静态页 + sitemap）、把阶段 A 快照灌库的 seed 脚本。本机以 SQLite 实测：接口对拍 47 项全通过（`node tests/api-check.mjs`）。
- **后台管理界面（2026-09-11 新增）**：`/admin` 可登录操作——概览、稿件列表（栏目用导航条，顺序与前台一致）、新建/编辑/删除稿件（新建默认“已发布”）、附件与正文插图上传、栏目管理、一键发布；会话 Cookie + CSRF + `password_hash`，登录与改动写操作日志。实测 214 项检查全通过（`node tests/admin-check.mjs`）。尚未做：栏目新建与删除、301 生成、编校微服务（`services/proofreader/` 仍为空骨架）。
...
- **检查脚本（2026-09-12）**：`tests/` 现有 7 个脚本，最近一次全绿——页面 19／20、接口 47、后台 223、排序 19、正文清洗 33＋31、编辑器 55。
...
node tests/api-check.mjs                                    # 47 项接口检查
...
│   ├── bin/               # migrate（建表）· seed（快照灌库）· publish（静态化发布）· scan-content（存量正文体检）· user（账号）
...
└── tests/                 # 七个检查脚本：页面／接口／后台／排序／正文清洗两层／富文本编辑器
```

> 标注“规划”的为预留骨架，尚无实现代码；已实现的为 `frontend/home/`（前端静态版）、`backend/`（后端最小骨架）、`database/`（建表脚本）、`tests/`（七个检查脚本）。
