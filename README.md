...
- **政协委员在线提交提案系统（2026-09-14 新增，一期：在线提交与提案委受理）**：主站内新增独立门户 **`/member`**——委员用提案委开通的账号登录后按标准提案表在线填写提交（案由、提案人类别、界别、专委会、联系电话、提案类别、情况与问题／分析／建议三段正文、附件），提案委在现有后台的“提案管理”里收件、受理或退回补充，委员随时查看办理状态与提案委意见、被退回的可改稿重交、可下载本人提案 Word 版。委员账号独立成表（`sys_member`，与后台 `sys_user` 隔离）、门户会话独立（`zx_member`），委员进不了内容后台；提案内容不对外公开，附件落在 `backend/storage/proposals/`（不在 webroot 下），只能经鉴权下载。名册用 CSV 导入（UTF-8／GBK 自动识别，表头 `姓名,界别,职务,联系电话`，姓名与职务必填），零星增补可在委员管理页「手工新建账号」里手工建一条或多条（与导入同一套校验与建号规则），系统生成随机初始密码并**只在下发时显示一次**，委员首次登录强制改密；收件清单可导出 Excel、单件可导出 Word 提案表（A4、案由三号居中、正文小四宋体、固定行距 28 磅，生产镜像已补 `zip` 扩展）。入口：首页首屏“提案填报系统”条幅改指 `/member`，“政协提案”栏目组右上角加“进入提案填报系统”。本期不含案号编排、交办与答复、征集批次与截止、提案公开、联名附议签名、承办单位账号与履职平台对接。设计与字段见 [docs/提案系统设计说明.md](docs/提案系统设计说明.md)，实现见 [backend/README.md](backend/README.md) 的“政协委员提案系统”一节，检查见 `node tests/proposal-check.mjs`。
...
- **检查脚本（2026-09-15）**：`tests/` 现有 11 个脚本，最近一次全绿——页面 28（接口模式 30、`--publish` 另加 2）、接口 55、后台 265、排序 19、正文清洗 33＋31、归一化 36、编辑器 62、旧地址 301 54、旧库迁移 49、提案系统 63。
