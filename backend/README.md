...
| 稿件管理 | 列表：**一块筛选面板**收稿库（导航条）+ 栏目（导航条）+ 关键词 + 排序（发布时间／最近更新／稿件号）+ 每页条数（20／50／100），标题即编辑入口，**批量提交／审核通过／退回／撤回／重新发布／恢复**（单次 ≤100篇，按权限位出动作），分页含页码与跳转，空结果给下一步；新建稿件（先点导航条选栏目，再填内容，状态默认“已发布”）；编辑页两栏：左侧一张“摘要与正文”写作窗（标题带64字计数、原标题三项与来源、正文提示、附件素材条、工具栏贴正文框的富文本正文），右侧只剩稿库流转、稿件信息与回收站，保存条吸底，正文可预览并显示字数；**提交环节不再设置任何排序**（置顶已从编辑页移除），列表里的**预览**打开前台正式详情页（`/detail.html?id=`）、**复制链接**给发布器产出的对外地址 `/article/{id}.html`，两者都只对“已发布 + 公开发布”的稿件开放，未发布与归档稿置灰（归档稿的提示说明不对外发布），**高亮／徽标**改到首页管理维护；删除稿件走确认页（软删除，可恢复） |
| 附件与插图 | 图片与附件的提交都在左侧写作窗里：插图走工具栏的“图片”按钮（`POST /admin/media/image`，**插在光标处**，文首／文中／文末都行；mp4／webm视频同理由工具栏的“视频”按钮走 `POST /admin/media/video`），附件走正文提示下面的素材条（pdf／doc／docx／xls／xlsx／ppt／pptx／zip／rar／txt，单个 ≤32 MB，上传后在文末“附件下载”区展示，可删除）。新建稿件还没有稿件号时，素材先落 `backend/public/uploads/pending/`，保存稿件时自动迁进 `uploads/<稿件号>/` 并登记图集。**图片一律 ≤ 2 MB**（横幅、头条大图、编辑器插图都算），附件与视频仍是32 MB |
