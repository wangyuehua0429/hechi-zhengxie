...
| 头条轮换 | 首屏大图轮播：**从已发布稿件里选**（自动带标题／摘要／链接，图缺省取稿件配图）或**手工新增外链条目**；上移／下移、上下线、删除，附首屏效果预览 |
| 其他栏目 | 首页正文 13 个内容模块的绑定维护：改模块标题、绑定栏目（指定栏目／一级栏目含子栏目／按子栏目分标签）、取几条、上下线，并预览当前取到的稿件；页面下方列未进导航的栏目 |
...
**首页稿件**：首页 13 个内容模块的列表由“其他栏目”里配置的绑定栏目**实时组装**（置顶在前 → 栏目内排序值 → 发布时间），不足时用同名快照兜底；稿件编辑页的“在本栏目置顶”写 `cms_article_channel.is_top`，首页模块与栏目页列表共用这一套顺序；稿件列表在按单个栏目筛选时提供 ↑ ↓ 调序。
