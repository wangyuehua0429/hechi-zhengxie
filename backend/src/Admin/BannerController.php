...
{
    /**
     * 槽位定义：key => [位置说明, 位置提示, 出图建议]。
     *
     * 出图建议里的「显示」是 2026-09-12 在 1440 与 1920 两档视口量出来的外框尺寸
     * （站点容器 `--maxw: 1400px`，视口再宽也不变大）；建议像素按显示尺寸的 2 倍取整，
     * 300 KB 的上限参照站内现有横幅图的量级（9—167 KB）。
     */
    public const SLOTS = [
        'hero-1' => ['首屏专题条幅 · 左', '与右侧等宽，横图', '显示 482×103；比例 355:76（约 4.7:1），建议 1000×214，≤ 300 KB'],
        'hero-2' => ['首屏专题条幅 · 右', '与左侧等宽，横图', '显示 482×103；比例 355:76（约 4.7:1），建议 1000×214，≤ 300 KB'],
        'body-1' => ['政协动态下方（通栏插画，可不填链接）', '通栏横图', '显示 978×174；比例约 5.6:1，建议 1960×350，≤ 300 KB'],
        'body-2' => ['时政要闻下方', '通栏横图', '显示 978×152；比例约 6.4:1，建议 1960×304，≤ 300 KB'],
        'body-3' => ['网上书院下方', '侧栏宽横图', '显示 400×186；比例约 2.2:1，建议 800×372，≤ 300 KB'],
        'body-4' => ['政协视频卡内', '侧栏宽横图', '显示 358×225；比例约 1.6:1，建议 716×450，≤ 300 KB'],
        'body-5' => ['三列模块上方（乡村振兴委员行，通栏）', '通栏横图', '显示 1400×142；比例约 9.9:1，建议 2800×283，≤ 300 KB'],
    ];
    ...
    public function index(Request $request): HtmlResponse|RedirectResponse
    {
        ...
        $slots = [];
        foreach (self::SLOTS as $key => [$label, $hint, $spec]) {
            $slots[] = [
                'slot'  => $key,
                'label' => $label,
                'hint'  => $hint,
