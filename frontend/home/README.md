...
“政协领导”页（`channel.html?id=202`）参照湖北省政协领导页做法：按“主席 → 副主席 → 秘书长”分组，每位一张“照片 + 姓名”卡片，卡片和姓名下的“简介 >>”都指向个人简介正文（`detail.html?id=<稿件ID>`）。排序与分组名单取自 `data/home.json` 的 `leaders`，不按发布时间倒序；这 10 位领导的简介正文与照片一并从旧站静态页抽取，保证点开即有内容。
