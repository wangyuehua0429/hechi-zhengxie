...
- 从内页点“首页”返回：`index.html` 的 `<head>` 内联脚本按来源页判断，给 `<html>` 加上 `mast-folded`（首屏即收窄，不闪全幅），`js/header-fold.js` 再展开回全幅，即“只有点首页才显示当前界面”。展开时先挂过渡再解除收缩态，标志放大与站头下移同时起止（不会等站头到位再放大标志）。
